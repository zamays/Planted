...
def settings():
    """
    Account settings page for location, password, and email management.

    GET: Display settings form
    POST: Process settings updates based on form_type parameter
    """
    if session.get('is_guest'):
        flash("Account settings are not available in guest mode. Please create an account.", "warning")
        return redirect(url_for('dashboard'))
    ...
    if request.method == "POST":
        ...
        if form_type == "location":
            ...
            try:
                ...
                # Update location service with new location
                location_service.set_manual_location(
                    latitude, longitude,
                ...
                flash("Invalid location coordinates. Please enter valid numbers.", "error")
            ...
            try:
                success = auth_service.change_email(user_id, new_email)
                if success:
...
@app.route("/api/detect_location", methods=["POST"])
def detect_location():
    """
    ...
    """
    if session.get('is_guest'):
        return jsonify({"status": "error", "message": "Not available in guest mode"})
