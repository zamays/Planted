...
def dashboard():
    ...
    try:
        ...
        due_tasks = garden_db.get_care_tasks(due_within_days=7) if garden_db is not None else []
        
        location_text = location_service.get_location_display() if location_service is not None else "Unknown Location"
        current_season = SeasonCalculator.get_current_season(
            location_service.current_location['latitude'] 
            if location_service is not None and location_service.current_location else 40.0
        )
        ...
        return render_template('dashboard.html', 
                             ...
                             weather=weather_service.current_weather if weather_service is not None else None)
    except Exception as e:
        print(f"Dashboard error: {e}")
        return f"<h1>Dashboard Error</h1><p>{str(e)}</p><p>Check console for details.</p>"
...
def plants():
    ...
    try:
        ...
        if type_filter != 'all':
            plants_list = [p for p in plants_list if p.plant_type.lower() == type_filter.lower()]

        # Apply search filter (matches name or scientific name)
        if search:
            plants_list = [p for p in plants_list if search.lower() in p.name.lower() or search.lower() in p.scientific_name.lower()]
        ...
        if isinstance(climate_zone, int):
            recommendations = SeasonCalculator.get_seasonal_recommendations(current_season, climate_zone)
        else:
            # Default zone 7 for unknown locations
            recommendations = SeasonCalculator.get_seasonal_recommendations(current_season, 7)
...
def add_plant():
    ...
    try:
        if request.method == 'POST':
            ...
            companion_plants = [p.strip() for p in companion_plants_str.split(',') if p.strip()]
            ...
            if climate_zones_str:
                try:
                    climate_zones = [int(z.strip()) for z in climate_zones_str.split(',') if z.strip()]
                except ValueError:
                    return "<h1>Error</h1><p>Climate zones must be numbers separated by commas (e.g., 5,6,7)</p>"
...
def view_plot(plot_id):
    ...
    try:
        ...
        for item in planted_items:
            plant = plant_db.get_plant_by_id(item.plant_id) if plant_db is not None else None
            if plant:
                items_with_plants.append({
                    'item': item,
                    'plant': plant
                })
        ...
        return render_template('view_plot.html',
                             plot=plot,
                             planted_items=items_with_plants)
    except Exception as e:
...
def plant_to_plot(plot_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            x_position = int(request.form.get('x_position'))
            y_position = int(request.form.get('y_position'))
            notes = request.form.get('notes', '').strip()

            # Validate position is within plot bounds
            if x_position < 0 or x_position >= plot.width or y_position < 0 or y_position >= plot.height:
            ...
            for item in existing_items:
                if item.x_position == x_position and item.y_position == y_position:
                    return "<h1>Error</h1><p>This position is already occupied. Please choose another square.</p>"
        ...
        return render_template('plant_to_plot.html',
                             plot=plot,
    ...
    try:
        ...
        if garden_db is None:
            ...
            if filter_type == 'today':
                tasks = [t for t in garden_db.get_care_tasks(due_within_days=1) 
                ...
                tasks = [t for t in all_tasks if t.due_date < datetime.now() and not t.completed]
            else:
        ...
        current_weather = weather_service.current_weather if weather_service is not None else None
...
def weather():
    ...
    try:
        current_weather = weather_service.current_weather if weather_service is not None else None
        forecast = weather_service.forecast if weather_service is not None and weather_service.forecast else []
        location_text = location_service.get_location_display() if location_service is not None else "Unknown Location"
        ...
        current_weather = weather_service.current_weather if weather_service is not None else "Unavailable"
...
def complete_task():
    ...
    try:
        ...
        if garden_db is None:
            return jsonify({'status': 'error', 'message': 'Garden database is not initialized.'})
