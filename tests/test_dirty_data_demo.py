...
def test_malformed_json():
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            _ = json.load(f)  # Don't need the data, just checking if it parses
        print("❌ UNEXPECTED: Malformed JSON was parsed successfully!")
        print("   The file should contain syntax errors.")
        return False
    except json.JSONDecodeError as e:
        print("✅ EXPECTED: JSON parsing failed with error:")
        print(f"   {e}")
        return True
    ...
    for filename in test_files:
        ...
        try:
            ...
            if "plants" not in data:
                print("   ⚠️  WARNING: No 'plants' key found")
...
def test_special_characters():
    ...
    try:
        ...
        for i, plant in enumerate(data.get("plants", []), 1):
            ...
            has_unicode = any(127 < ord(c) < 127000 for c in name)
            has_injection = any(pattern in name.lower()
                              for pattern in ["sql", "drop", "script", "alert"])

            if has_emoji:
                print("   📱 Contains emoji characters")
            if has_unicode:
                print("   🌐 Contains Unicode characters")
            if has_injection:
                print("   ⚠️  Contains potential injection patterns")
